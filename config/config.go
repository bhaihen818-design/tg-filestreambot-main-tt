...
type config struct {
	...
	// GDPlayer control-plane configuration. The bot sends metadata only; it never
	// uses this integration to transfer Telegram file bytes.
	GDPlayerControlURL   string `envconfig:"GDPLAYER_CONTROL_URL"`
	GDPlayerSharedSecret string `envconfig:"GDPLAYER_SHARED_SECRET"`
	GDPlayerTimeoutSec   int    `envconfig:"GDPLAYER_TIMEOUT_SEC" default:"30"`
}
...
func Load(log *zap.Logger, cmd *cobra.Command) {
	...
	if ValueOf.GDPlayerTimeoutSec <= 0 {
		log.Sugar().Info("GDPLAYER_TIMEOUT_SEC must be greater than 0, defaulting to 30 seconds")
		ValueOf.GDPlayerTimeoutSec = 30
	}
	if ValueOf.GDPlayerControlURL == "" || ValueOf.GDPlayerSharedSecret == "" {
		log.Warn("GDPlayer control integration is not configured; media requests will not create player links")
	}
}
